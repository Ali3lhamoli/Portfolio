        ...
        <div className="max-w-4xl mx-auto">
          <div className="text-center mb-16">
            ...
            <p className="text-lg text-gray-600 dark:text-gray-300 max-w-2xl mx-auto transition-colors duration-300">
              Building robust backend solutions and collaborating with cross-functional teams 
          ...
          <div className="space-y-8">
            {experiences.map((exp, index) => (
              <div key={index} className="bg-white dark:bg-gray-900 rounded-xl shadow-lg dark:shadow-black/50 p-6 md:p-8 relative overflow-hidden transition-all duration-300 hover:shadow-xl hover:shadow-cyan-500/10">
                {exp.current && (
                ...
                <div className="flex flex-col md:flex-row md:items-start md:justify-between mb-6">
                  <div className="flex-1">
                    <div className="flex items-center space-x-3 mb-2">
                      ...
                      <div>
                        <h3 className="text-xl font-bold text-gray-900 dark:text-white transition-colors duration-300">{exp.position}</h3>
                        <p className="text-lg text-cyan-600 dark:text-cyan-400 font-semibold transition-colors duration-300">{exp.company}</p>
                      </div>
                    </div>
                    
                    <div className="flex flex-wrap gap-4 text-sm text-gray-600 dark:text-gray-300 mb-4 transition-colors duration-300">
                      ...
                      <span className="bg-gray-100 text-gray-700 px-2 py-1 rounded-full text-xs uppercase">
                        {exp.type}
                      </span>
                    </div>
                  </div>
                ...
                <div>
                  <h4 className="font-semibold text-gray-900 dark:text-white mb-3 transition-colors duration-300">Key Responsibilities:</h4>
                  <ul className="space-y-2">
                    {exp.responsibilities.map((resp, respIndex) => (
                      <li key={respIndex} className="flex items-start space-x-3">
                        <div className="w-2 h-2 bg-gradient-to-r from-cyan-500 to-purple-600 rounded-full mt-2 flex-shrink-0"></div>
                        <span className="text-gray-700 dark:text-gray-300 transition-colors duration-300">{resp}</span>
                      </li>
                    ))}
                  </ul>
                </div>
